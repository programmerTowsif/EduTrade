const buyOrders = [
  ...
  },
  {
    ...
    budget: 1200,
    condition: "Complete kit preferred",
    user: "Sabbir Hasan",
    time: "Yesterday",
    tag: "EEE",
  },
  {
    ...
    category: "Books",
    budget: 450,
    condition: "Latest edition preferred",
    user: "Afsana Islam",
    time: "Yesterday",
    tag: "Architecture",
  },
  {
    ...
    category: "Notes",
    budget: 200,
    condition: "Clear photocopy is fine",
    user: "Tania Sultana",
    time: "2 days ago",
    tag: "Math",
  },
...
function renderOrders() {
  ...
  let orders = buyOrders.filter(
    (order) =>
      (category.value === "all" || order.category === category.value) &&
      `${order.title} ${order.category} ${order.tag}`
        .toLowerCase()
        .includes(query),
  );
  if (sort.value === "budget")
    orders = [...orders].sort((a, b) => b.budget - a.budget);
